...
export function Chapter3Page() {
  ...
  const totalHeight = chunkPositions.at(-1)?.bottom || 0;
  ...
  const visibleChunkIndices = useMemo(() => {
    ...
    chunkPositions.forEach((chunk, index) => {
      ...
      if (chunk.bottom >= viewportTop && chunk.top <= viewportBottom) {
      ...
      if (chunkData.type === "title") {
        return (
          <div
            ...
            style={{ top: chunkData.top, height: chunkData.height + 2 }}
          >
            <img
              src={nenHong2}
              ...
              style={{
                ...
                height: chunkData.height + 4,
                width: "100%",
                transform: "translateY(-2px)",
              }}
            />
            <div className="relative z-10 text-center px-6 pr-16 w-full max-w-6xl">
              <h1 className={`text-4xl md:text-6xl font-bold mb-8 text-white drop-shadow-lg transition-all duration-1500 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
                {t("title")}
              </h1>
              <p className={`text-xl md:text-2xl text-white max-w-4xl mx-auto leading-relaxed transition-all duration-1500 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`} style={{ transitionDelay: "500ms" }}>
                {t("description")}
              </p>
            </div>
          </div>
        ...
        >
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 pr-16 w-full h-full flex items-center">
            <div className={`grid grid-cols-1 ${hasImages ? "lg:grid-cols-2" : "lg:grid-cols-1"} gap-12 items-center w-full min-h-[80vh]`}>
              <div className={`space-y-6 ${hasImages ? (contentOnLeft ? "lg:order-1" : "lg:order-2") : "lg:col-span-1 max-w-4xl mx-auto"} transition-all duration-1200 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-16"}`} style={{ transitionDelay: "300ms" }}>
      ...
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-40 flex space-x-2">
        {chunkPositions.map((chunkData) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-all duration-300 border border-white/30 ${visibleChunks.has(chunkData.id) ? "bg-white scale-150 shadow-lg border-white" : "bg-white/40 hover:bg-white/60 hover:scale-125"}`}
