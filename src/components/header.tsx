...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 flex justify-center py-3 px-4">
      <div
        className={`w-full max-w-6xl bg-slate-800/70 backdrop-blur-md shadow-sm transition-all duration-300 rounded-none ${
      ...
      >
        ...
        <div className="flex items-center justify-between h-14 px-4">
          ...
          {/* Desktop nav */}
          <nav className="hidden md:flex items-center gap-4 oswald">
            {navItems.map(({ key, path }) => (
              <Link
                ...
                } after:absolute after:left-0 after:bottom-0 after:h-[2px] after:w-full after:bg-white after:scale-x-0 after:transition-transform after:duration-300 hover:after:scale-x-100`}
              >
                {t(key)}
              </Link>
            ))}
        ...
        >
          <nav className="flex flex-col gap-2 px-4 max-h-[70vh] overflow-y-auto oswald">
            {navItems.map(({ key, path }) => (
              <Link
                key={key}
                to={path}
